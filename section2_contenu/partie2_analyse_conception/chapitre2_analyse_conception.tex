...
\begin{itemize}
	\item \textbf{Visiteur :} utilisateur non connecté, sans compte sur la plateforme. Il peut consulter «~SALISA~» et rechercher des prestataires, mais son seul point d'entrée vers les fonctionnalités complètes est l'inscription, qui lui permet de devenir un utilisateur à part entière ;
...
\begin{itemize}
	...
	\item UC-04 : se connecter (Demandeur, Prestataire, Administrateur) ;
...
La relation \textbf{$\langle\langle$include$\rangle\rangle$} indique qu'un cas d'utilisation est toujours exécuté dans le cadre d'un autre. Par exemple, UC-01 (s'inscrire) inclut systématiquement UC-02 (valider le code OTP), car on ne peut pas créer de compte sans valider le code reçu par SMS. De même, UC-21 (accepter un devis) inclut UC-23 (payer via Mobile Money), car l'acceptation d'un devis déclenche toujours le processus de paiement.

La relation \textbf{$\langle\langle$extend$\rangle\rangle$} indique qu'un cas d'utilisation est exécuté de façon optionnelle dans certaines conditions. Par exemple, UC-11 (publier une mission) peut optionnellement étendre vers UC-12 (utiliser l'assistant IA), selon le choix du demandeur. De même, UC-26 (valider la mission) peut étendre vers UC-27 (scanner le code QR), uniquement pour les missions réalisées en présentiel.
