...
% Un profil prestataire est une vitrine professionnelle publique : il comprend
% un portfolio, des tarifs, des zones d'intervention, des compétences et des
% badges de certification. Ces éléments n'ont de sens que pour celui qui propose
% des services et cherche à convaincre des clients potentiels. Un demandeur
% n'a pas besoin de se présenter professionnellement : son seul engagement est
% de publier une mission et de payer. C'est pourquoi ce package porte le nom
% de « Profil prestataire » et non « Gestion de profil » au sens générique.
% ---------------------------------------------------------------
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}|p{10.5cm}|}
		...
		\hline
		\textbf{UC-10} & soumettre une demande de vérification d'identité pour obtenir le badge Vérifié (Prestataire, Administrateur) \\
		\hline
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3cm}|p{10.5cm}|}
		\hline
		\multicolumn{2}{|c|}{\cellcolor{vertCongo}\textcolor{white}{\textbf{Package P6 : Paiement et escrow}}} \\
		\hline
		\rowcolor{vertCongo}
		\textcolor{white}{\textbf{Cas d'utilisation}} & \textcolor{white}{\textbf{Définition}} \\
		\hline
		...
		\hline
	\end{tabular}
	\caption{Cas d'utilisation du package P6 : Paiement et escrow}
	\label{tab:cu_p6}
\end{table}
...
Les relations entre cas d'utilisation permettent de structurer et de clarifier les dépendances entre les fonctionnalités du système \cite{roques_uml}. On en distingue deux types principaux. La relation \textbf{$\langle\langle$include$\rangle\rangle$} indique qu'un cas d'utilisation est toujours exécuté dans le cadre d'un autre : par exemple, UC-01 (s'inscrire) inclut systématiquement UC-02 (valider le code OTP), car on ne peut pas créer de compte sans valider le code reçu par SMS ; de même, UC-21 (accepter un devis) inclut UC-23 (payer via Mobile Money), car l'acceptation d'un devis déclenche toujours le processus de paiement. La relation \textbf{$\langle\langle$extend$\rangle\rangle$}, quant à elle, indique qu'un cas d'utilisation est exécuté de façon optionnelle dans certaines conditions : par exemple, UC-11 (publier une mission) peut optionnellement étendre vers UC-12 (utiliser l'assistant IA pour la description de la mission), selon le choix du demandeur, et UC-26 (valider la mission) peut étendre vers UC-27 (scanner le code QR), uniquement pour les missions réalisées en présentiel.

\vspace{0.4cm}
Avant de détailler chaque domaine fonctionnel, il est utile de disposer d'une vue d'ensemble du système. Cependant, représenter les 39 cas d'utilisation dans un seul diagramme produirait un résultat illisible. Nous avons donc choisi de présenter d'abord un diagramme de packages, qui montre les neuf domaines fonctionnels de « SALISA » et leurs relations avec les acteurs. Ce diagramme met notamment en évidence une dépendance commune à l'ensemble des packages : tous requièrent que l'utilisateur soit préalablement authentifié (P1 - Authentification), ce qui est représenté par des relations $\langle\langle$include$\rangle\rangle$ convergeant vers P1. La figure~\ref{fig:diag_packages} illustre ce diagramme.
...
Pour une meilleure lisibilité, chaque package est ensuite détaillé dans un diagramme de cas d'utilisation dédié, présentant les cas d'utilisation et leurs relations avec précision. Les figures~\ref{fig:cu_p1} à~\ref{fig:cu_p9} illustrent ces neuf diagrammes.
...
\begin{figure}[H]
	...
	\caption{Diagramme de cas d'utilisation - P6 : Paiement et escrow}
	\label{fig:cu_p6}
\end{figure}
