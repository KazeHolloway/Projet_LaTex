...
\vspace{0.4cm}
\noindent\textbf{\textcolor{vertCongo}{a- Solution 1 : portail web de mise en relation avec annuaire de profils et messagerie intégrée (PHP, MySQL, Bootstrap)}}
...
\vspace{0.2cm}
\textit{\textbf{\textcolor{vertCongo}{Avantages :}}}
\begin{itemize}
...
\vspace{0.1cm}
\textit{\textbf{\textcolor{vertCongo}{Inconvénients :}}}
\begin{itemize}
...
\vspace{0.4cm}
\noindent\textbf{\textcolor{vertCongo}{b- Solution 2 : développement d'une application web cross-plateforme (PWA-like) avec matching intelligent et paiement sécurisé}}

\vspace{0.3cm}
Il s'agit de développer une application web, accessible depuis tous les terminaux sans installation obligatoire. Elle intègre des fonctionnalités complètes, à savoir la gestion des profils, la publication de missions, la messagerie, la mise en correspondance intelligente entre prestataires et clients, le règlement des missions via MTN MoMo ou Airtel Money, et le système de notation.
...
\begin{itemize}
	...
	\item intègre le paiement mobile via MTN MoMo et Airtel Money, adapté au contexte local \cite{mtn_momo} ;
	\item algorithme de mise en correspondance pour aider les clients à trouver le bon prestataire rapidement ;
	\item système de notification multi-canaux pour tenir les utilisateurs informés en temps réel.
\end{itemize}
...
\vspace{0.4cm}
\noindent\textbf{\textcolor{vertCongo}{c- Solution 3 : développement d'une application mobile native de mise en relation entre prestataires et clients (React Native)}}
...
\textit{\textbf{\textcolor{vertCongo}{Avantages :}}}
\begin{itemize}
	\item expérience utilisateur optimale sur chaque système, grâce à l'accès complet aux fonctionnalités natives du terminal ;
...
Notre choix se porte sur la \textbf{solution 2}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.65\textwidth]{logo_uml_concepts.png}
	\caption{Les concepts clés du langage UML}
	\label{fig:uml_concepts}
\end{figure}

\textbf{UML} (\textit{Unified Modeling Language}, ou Langage de Modélisation Unifié) est un langage graphique standardisé par l'OMG (\textit{Object Management Group}) depuis 1997 \cite{booch_uml}. UML est reconnu comme norme ISO/IEC 19505, il constitue aujourd'hui la référence mondiale pour la modélisation des systèmes logiciels orientés objet.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.90\textwidth]{uml_types_diagrammes.png}
	\caption{Les types de diagrammes UML}
	\label{fig:uml_types}
\end{figure}

Contrairement à la version 1.x d'UML (1997-2003) qui ne proposait qu'environ 9 types de diagrammes principaux (use case, classes, objets, séquence, collaboration, état, activité, composants, déploiement), la version 2.x propose \textbf{quatorze types de diagrammes}, répartis en deux grandes familles \cite{booch_uml}.

\vspace{0.3cm}
\noindent\textbf{\textcolor{vertCongo}{a- Diagrammes structurels (vue statique)}}
...
\vspace{0.3cm}
\noindent\textbf{\textcolor{vertCongo}{b- Diagrammes comportementaux (vue dynamique)}}

\vspace{0.2cm}
Ces diagrammes décrivent le comportement du système dans le temps. Parmi eux, on trouve entre autres le diagramme de cas d'utilisation, qui identifie les acteurs et les fonctionnalités qu'ils peuvent utiliser ; le diagramme de séquence, qui montre les échanges entre objets dans le temps ; le diagramme de communication ; le diagramme d'activités, qui représente un enchaînement d'actions et de décisions dans un processus ; le diagramme d'états-transitions ; le diagramme de vue d'ensemble des interactions ; et le diagramme de temps.

Pour « SALISA », nous utiliserons principalement le \textbf{diagramme de contexte statique}, le \textbf{diagramme de cas d'utilisation}, le \textbf{diagramme de packages}, le \textbf{diagramme de séquence}, le \textbf{diagramme d'activités}, le \textbf{diagramme de classes}, le \textbf{diagramme d'objets}, et le \textbf{diagramme de déploiement}.
...
Le \textbf{Processus Unifié} (\textit{Unified Process}, abrégé \textbf{UP}) est un cadre de développement logiciel itératif et incrémental, formalisé par Ivar Jacobson, Grady Booch et James Rumbaugh à la fin des années 1990 \cite{jacobson_up}. Sa version commerciale la plus connue est le \textit{Rational Unified Process} (RUP).
...
Le \textbf{2TUP} (\textit{Two Tracks Unified Process}) est une adaptation du Processus Unifié, proposée par Pascal Roques et Franck Vallée dans leur ouvrage \textit{UML en action} \cite{roques_uml}. Sa particularité est son organisation en deux branches parallèles qui se rejoignent en phase de réalisation.

\vspace{0.4cm}
\noindent\textbf{\textcolor{vertCongo}{a- Branche fonctionnelle (What)}}
...
\vspace{0.4cm}
\noindent\textbf{\textcolor{vertCongo}{b- Branche technique (How)}}
...
\vspace{0.4cm}
\noindent\textbf{\textcolor{vertCongo}{c- Branche de réalisation (fusion)}}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{4cm}|p{3.5cm}|p{6cm}|}
		\hline
		\rowcolor{vertCongo}
		\textcolor{white}{\textbf{Phase 2TUP}} & \textcolor{white}{\textbf{Branche}} & \textcolor{white}{\textbf{Diagrammes UML utilisés}} \\
		\hline
		...
		\hline
		Conception du\newline domaine & Réalisation & Diagramme de classes, Diagramme d'objets \\
		\hline
	\end{tabular}
